...
function Btn({ children, primary, onClick, style={} }) {
  return <button onClick={onClick} style={{ height:32, padding:"0 14px", background:primary?C.accent:C.surface, border:`1px solid ${primary?C.accent:C.border2}`, borderRadius:6, fontSize:12, color:primary?"#fff":C.textMid, fontWeight:primary?500:400, cursor:"pointer", ...style }}>{children}</button>;
}
  ...
  return (
    <div>
      <div style={{ display:"flex", gap:8, marginBottom:14 }}>
        <Btn primary={tab==="usage"} onClick={() => setTab("usage")}>📋 로봇 사용 기록</Btn>
        <Btn primary={tab==="errors"} onClick={() => setTab("errors")}>⚠️ 오류/비상정지 이력</Btn>
      </div>
      <div style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:6, boxShadow:"0 1px 3px rgba(0,0,0,.05)" }}>
        ...
        {tab==="usage" ? (
          ...
          <table style={{ width:"100%", borderCollapse:"collapse", fontSize:12 }}>
            <thead><tr>{["날짜","시간","연구원","유형","설명"].map(h=><th key={h} style={{ textAlign:"left", padding:"8px 12px", borderBottom:`2px solid ${C.border}`, color:C.textDim, fontSize:11, fontWeight:600 }}>{h}</th>)}</tr></thead>
            <tbody>{errors.map((e,i) => (
              <tr key={i} style={{ background:i%2?C.surface2:C.surface }}>
                <td style={{ padding:"7px 12px" }}>{e.date}</td><td style={{ padding:"7px 12px", fontFamily:"Consolas", fontSize:11 }}>{e.time}</td>
                <td style={{ padding:"7px 12px", fontWeight:500 }}>{e.researcher}</td>
                <td style={{ padding:"7px 12px" }}><Badge type={e.type==="비상정지"?"err":"warn"}>{e.type}</Badge></td>
                <td style={{ padding:"7px 12px", fontSize:11 }}>{e.desc}</td>
              </tr>))}</tbody>
          </table>
        )}
      </div>
