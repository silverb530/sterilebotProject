...
REM Python 버전 체크 (3.11 인지)
python -c "import sys; exit(0 if sys.version_info[:2]==(3,11) else 1)" >nul 2>&1
if errorlevel 1 (
    echo [WARNING] Python 3.11 이 아닙니다.
    echo dlib whl 파일이 cp311 (Python 3.11) 용이라 호환 안 될 수 있습니다.
    echo 그래도 계속하시려면 아무 키나 누르세요. 중단하려면 Ctrl+C.
    pause
)
...
REM 4. dlib 별도 설치 (wheels 폴더에서)
echo [4/5] dlib 설치 (wheels 폴더에서)...
set DLIB_WHL=
for %%f in ("%~dp0wheels\dlib-*.whl") do set DLIB_WHL=%%f
if "%DLIB_WHL%"=="" (
    echo [ERROR] wheels 폴더에 dlib whl 파일이 없습니다.
    echo wheels\dlib-19.24.1-cp311-cp311-win_amd64.whl 같은 파일이 필요합니다.
    call deactivate
    pause
    exit /b 1
)
echo dlib whl 파일: %DLIB_WHL%
pip install "%DLIB_WHL%"
if errorlevel 1 (
    echo [ERROR] dlib 설치 실패. Python 버전이 3.11 인지 확인하세요.
    call deactivate
    pause
    exit /b 1
)

REM 5. server 나머지 라이브러리 설치 (face_recognition 등 — dlib 이미 있으니 잘 됨)
...
echo      → camera_indices.json 자동 생성
echo   2. calib.py --name 영문이름 실행
echo      → calib_data/영문이름.npy 생성
